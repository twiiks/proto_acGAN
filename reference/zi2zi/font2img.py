...
DEFAULT_CHARSET = "./charset/kr.json"


def load_global_charset():
    global KR_CHARSET
    krset = json.load(open(DEFAULT_CHARSET))
#    KR_CHARSET = kr["kr"]
    for _ in krset["kr"]:
        KR_CHARSET.append(chr(_))
#    print(KR_CHARSET) # ['가', '나', ...]
...
def draw_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
    """
    draws src_font, dst_font in one img
    """
    dst_img = draw_single_char(ch, dst_font, canvas_size, x_offset, y_offset)
    # check the filter example in the hashes or not
    dst_hash = hash(dst_img.tobytes()) # str -> value as 19bit int
...
parser.add_argument('--char_size', dest='char_size', type=int, default=200, help='character size')
...
parser.add_argument('--label', dest='label', type=int, default=1, help='label as the prefix of examples')

args = parser.parse_args()

if __name__ == "__main__":
    if args.charset == 'KR':
        print("charset is :%s" %args.charset)
        charset = locals().get("%s_CHARSET" % args.charset)
#        charset = locals().get(KR_CHARSET)
#        print(charset) # ['가','나', ...]
    else:
        print("else loop")
#        print(args.charset) ###KR 
